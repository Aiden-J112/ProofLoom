...
- 在界面中“导入 Markdown”是服务器进程从 `browse_root` 下读取本地文件，不是上传到云端；原始 Markdown 文件本身不会被复制进仓库。
- `browse_root` 是 UI 可选择项目和来源的路径边界。默认示例配置把它设为配置文件所在目录（仓库根目录）。
- 解析出的 Source Fragment（包含原文片段）会保存到 Knowledge Project 的 `.proofloom/`。项目数据也应按敏感资料保护，不得提交或公开；创建项目时，ProofLoom 会把 `.proofloom/` 加入该项目自己的 `.gitignore`。
